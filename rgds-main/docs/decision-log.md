...
## v1.2 additions: making IND reality explicit

These fields were added to reflect real IND execution challenges raised by webinar speakers (e.g., “drafting at risk,” reviewer triage, scope volatility, and regulatory interaction strategy).

### Author-at-risk items (placeholders)
**Prevents:** “placeholder drafting” becoming invisible risk.

Use `author_at_risk_items[]` when you draft with missing inputs (e.g., a final study report not yet released). Each item must specify:
- what is placeholder
- how it will be verified
- when it must close
- who owns closure
- what happens if it misses (fallback)

### Review plan (triage routing)
**Prevents:** late-breaking changes failing because “everyone” was asked to review.

Use `review_plan` to explicitly name:
- required reviewers (must review)
- optional reviewers (inform / optional)
- timebox and escalation path

If `author_at_risk_items[]` is present, `review_plan` is required.

### Scope change events
**Prevents:** scope surprises being handled informally.

Use `scope_change_events[]` to record late-discovered reports, new studies, protocol changes, or stakeholder requests that force replanning. Each event records impact, owner, and resolution.

### Regulatory interaction decision
**Prevents:** pre-IND / agency strategy being implicit or reconstructed later.

Use `regulatory_interaction_decision` to capture meeting type, rationale, questions, timing window, and contingency if the meeting does not happen.

### Fallback plan (required for conditional_go)
**Prevents:** conditional approvals without a “what if we miss?” plan.

When `decision_outcome.outcome` is `conditional_go`, `fallback_plan` is required. It should define triggers, actions, owner, and rough time/cost impact.
